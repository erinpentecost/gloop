...
func main() {
	...
	window, err := glfw.CreateWindow(int(reqDim.Width), int(reqDim.Height), "VulkanCube (GLFW and gloop)", nil, nil)
	orPanic(err)
	...
	loop, _ := gloop.NewLoop(nil, nil, gloop.Hz60Delay, gloop.Hz60Delay)
	...
	render := func(step time.Duration) error {
		...
		// outdated is true if the window changed shape or something,
		// indicating that it should be redrawn.
		if outdated {
	...
	loop.Render = render
	loop.Simulate = simulate

	metrics := NewMetricsServer()
	metrics.Serve(loop.Done())
